...
<h2 class="contentTitle">SQL脚本修改</h2>
<div class="pageContent">
	<form id="myForm" method="post" action="${basePath}web/sqlScripts/saveOrUpdate"
		...
		<div class="pageFormContent nowrap" layoutH="120" id ="divId">
			...
			<input type="hidden" name="stepId" value="${bean.stepId }" />
			<input type="hidden" name="aOUpdate" value="1" />
			<dl>
				<dt>名称：</dt>
				<dd>
					<input type="text" name="name" maxlength="20" value="${bean.name }" class="required" style="width: 200px" />
				</dd>
			</dl>
			<dl>
				<dt>操作类型：</dt>
				<dd>
					<select id="type" name="type" style="width: 207px" disabled="disabled">
						<c:forEach var="entry" items="${map}">   
        					<option value="<c:out value="${entry.key}" />"  
        						<c:if test="${bean.type == entry.key}">selected="selected"</c:if> ><c:out value="${entry.value}"/></option>  
    					</c:forEach> 
					</select>
				</dd>
			</dl>
				...
				<dd>
					<select name="datasourceId" id="datasource" class="combox" onchange="toCheckSql('1')">
						<c:forEach items="${datasourceList}" var="list" varStatus="status">
							<option value="${list.id }"<c:if test="${list.id == bean.datasourceId}">selected="selected"</c:if>>${list.name }
							</option>
						</c:forEach>
					</select>
				</dd>
			...
			<dl>
				<dt>SQL语句：</dt>
				<dd>
					<textarea rows="10" cols="80" id="sql" name="sql" class="required" onblur="toCheckSql('2')">${bean.sql }</textarea>
				</dd>
				<span style="color: red" id="errorSpan"></span>
			</dl>
			<c:if test="${bean.paramMap != null }">
				<c:forEach var="entry" items="${bean.paramMap}">
				<dl class="add_">
				<dt><c:out value="${fn:split(entry.key,'#')[0]}" /><c:if test="${fn:split(entry.key,'#')[1] != null &&fn:split(entry.key,'#')[1] !=''}">@</c:if>：</dt>
				<dd>
					<input type='text' name='${entry.key}' style='width: 200px' value="<c:out value="${entry.value}" />" /></dd>
				</dd>
