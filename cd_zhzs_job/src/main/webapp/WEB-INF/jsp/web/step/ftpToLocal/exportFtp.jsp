...
<!-- 	<h2 style="margin-right: 90%">FTP文件操作</h2> -->
	<h2 class="contentTitle" style="margin-right: 93%;margin-left: -2%">FTP文件下载</h2>
</div>
<div class="pageContent">
	<form id="myForm" method="post" action="${basePath}web/ftp/saveToMongoDownLoad?jobId=${id}&stepId=${stepId}&sortNum=${sortNum}"
		...
		<div class="pageFormContent nowrap" layoutH="80">
			...
			<dl style="margin-top: 1%">
				<dt>FTP文件路径 ：</dt>
				<dd>
					<input name="filePath" type="text" class="required" style="width: 200px" value="${needStep.filePath}"/>
				</dd>
			</dl>
			<dl>
				<dt>文件本地存放路径 ：</dt>
				<dd>
					<input id="file" name="localPath" class="required noRootPath" type="text" style="width: 200px" value="${name}"/><span
				...
				<dd>
					<select name="fileNamePatterns" id="fileNamePatterns" class="combox" >
						...
						<c:forEach items="${fileNamePatternsMap}" var="map" varStatus="status">
							...
							<c:if test="${map.key == needStep.fileNamePatterns}">selected="selected"</c:if>
							>${map.value}</option>
						</c:forEach>
					</select>
					<span style="color: red;height:21px;line-height: 21px">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;可不选</span>
				</dd>
			</dl>
		</div>
		<div class="formBar">
