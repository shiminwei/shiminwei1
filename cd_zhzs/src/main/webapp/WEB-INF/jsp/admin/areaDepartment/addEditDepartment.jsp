...
<body>
	...
	<div class="pageContent">
		...
		<form method="post"
			action="${basePath }admin/department/addEditDepartment?type=${type}&departmentId=${sysDepartmentInfo.departmentId}&departmentNameq=${departmentName}"
			...
			<div class="pageFormContent nowrap" layoutH="58">
				<dl>
					...
					<dd>
						<input type="text" id="departmentName"
							value="${sysDepartmentInfo.departmentName }"
							name="departmentName" onblur="checkDepartmentName()" /> <strong><span
							id="checkdepartmentName"
						...
						<select class="combox" name="departmentAreaId">
							<c:forEach items="${areaList }" var="area">
								<option value="${area.areaId }"
									<c:if test="${sysDepartmentInfo.departmentAreaId == area.areaId}">selected="selected"</c:if>>${area.areaName }</option>
							</c:forEach>
						</select>
					</dd>
				</dl>
				<dl>
					...
					<dd>
						<select class="combox" name="parentDepartment" id="dd"
							onchange="changeParentDepartment(this)">
							<option id="parentDepartment" value="">--请选择--</option>
							<c:forEach items="${departmentList }" var="department">
								<option
									value="${department.departmentId}-${department.departmentCode}"
									<c:if test="${sysDepartmentInfo.parentDepartmentId == department.departmentId}">selected="selected"</c:if>>${department.departmentName }</option>
							</c:forEach>
						</select>
					</dd>
				</dl>
				...
				<dl>
					...
					<dd>
						...
						<input type="text" id="code" name="departmentCode"
							value="${sysDepartmentInfo.departmentCode }"
							<c:if test="${sysDepartmentInfo!=null}">readonly="true"</c:if>
							onblur="checkDepartmentCode('${sysDepartmentInfo.departmentCode }');" />
						<strong><span id="checkCode"
							style="font-size: 10px; line-height: 20px; color: red"></span></strong>
					</dd>
				</dl>
			</div>
				...
				<ul>
					<li><div class="buttonActive">
							<div class="buttonContent">
								<button type="button" id="button" onclick="toSave()">保存</button>
							</div>
						</div></li>
					<li><div class="button">
							<div class="buttonContent">
	...
	<script type="text/javascript">
		...
		function checkDepartmentCode(e) {
			if (e == "") {
