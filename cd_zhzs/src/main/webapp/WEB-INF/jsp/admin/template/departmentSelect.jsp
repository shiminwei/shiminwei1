...
<div class="pageContent" overflow: x:scroll;>
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="edit" onclick="selectTemplate();" rel="templateNames"  href="javascript:;" callback="dialogAjaxDoneFather" title="确定要选择吗?"><span>选择</span></a></li>
			<li><a class="delete" target="selectedTodo" targetType='dialog' rel="templateNames" href="${basePath }admin/template/unselectTemplate?departmentId=${departmentId }"  callback="dialogAjaxDoneFather" title="确定要取消选择吗?"><span>取消</span></a></li>
		</ul>	
	</div>
	<table class="table" width="100%" layoutH="115">
		<thead>
			<tr>
				<th><input type="checkbox" group="templateNames" class="checkboxCtrl"></th>
				<th>模版名称</th>
				<th>报送周期</th>
				<th>报送过期天数</th>
				<th>对应表</th>
		...
		<tbody>
			...
			<tr target="template_name" rel="${item.name }" <c:if test="${iscontain }">style='background: #e1e1e1;'</c:if>>
...
function selectTemplate(){
	...
	if(templateNames.length>0 && templatePeriodArray.length>0){
		$.ajax({
			...
			data:{"departmentId":departmentId,"templateNames":templateNamesArray,"reportPeroid":templatePeriodArray},
			dataType:"json",
			cache: false,
