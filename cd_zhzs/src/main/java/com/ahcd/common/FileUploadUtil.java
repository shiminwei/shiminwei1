...
public class FileUploadUtil {
	public final static String excelTemplatePath = PropertyManager.getConfigProperty("config_path")
			+ PropertyManager.getConfigProperty("upload_excel_path") + "excel/";
	public final static String noticeFilePath = PropertyManager.getConfigProperty("config_path")
			+ Constant.noticeFileUpload;
	 ...
	 * 
	   * 功能说明    : 文件下载
	   * @author   : fei yang 
	   * @version ：2016年11月2日 下午2:22:04 
	   * @param response
	   * @param fileName
	   * @param fileType
	   * @throws Exception
	 */
	public static void downloadFile(HttpServletResponse response, String filePath,String fileName, String fileType) throws Exception {
		// 读到流中
		InputStream inStream = new FileInputStream(filePath + fileName + fileType);// 文件的存放路径
		// 设置输出的格式
		response.reset();
		response.setContentType("application/vnd.ms-excel;charset=UTF-8");
		response.setHeader("Content-Disposition",
				"attachment; filename=" + new String(fileName.getBytes("gb2312"), "iso8859-1") + fileType);// 设定输出文件头
		// 循环取出流中的数据
		byte[] b = new byte[100];
		int len;
		try {
