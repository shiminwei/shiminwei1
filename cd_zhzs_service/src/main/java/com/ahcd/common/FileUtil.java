...
public class FileUtil {
	...
	// 拷贝文件:输入输出流已建好
	public static final boolean newCopyFile(BufferedInputStream bufferedInputStream, BufferedOutputStream bufferedOutputStream) throws Exception {
		try {
			byte[] buf = new byte[1024];
			int i = 0;
	 ...
	 * 
	 * @author : fei yang
	 * @version ：2016年11月3日 上午11:36:22
	 * @param file
	 * @param path
	 * @param name
	 * @param fileType
	 * @return re 0读取异常 -1文件不存在 -2 文件格式不正确
	 * @throws IOException
	 */
	public static int uploadFile(MultipartFile file, String path, String name, String fileType) throws IOException {
		int re = 0;
	...
	public static boolean vidateFileType(String fileName, String fileType) {
		if (!StringUtil.isBlank(fileName) && fileName.lastIndexOf(".") >= 0) {
	...
	/**
	 * 
	 * 功能说明 :判断文件夹是否存在 不存在则创建
	 * 
	 * @author : fei yang
	 * @version ：2017年4月18日 下午1:29:28
	 * @param filePath
	 */
	public static void isDirectoryOrMkdirs(String filePath) {
		File file = new File(filePath);
		if (!file.isDirectory()) {
			file.mkdirs();
		}
	...
	/**
	 * 
	 * 功能说明 : 根据文件夹路径获取子文件所有符合类型的文件
	 * 
	 * @author : fei yang
	 * @version ：2017年4月19日 下午3:10:36
	 * @param dirFilePath
	 * @param fileType
	 * @return
	 */
	public static List<String> getFileDirFilesByType(String dirFilePath, String fileType) {
		if (StringUtil.isNotEmpty(dirFilePath) && StringUtil.isNotEmpty(fileType)) {
			File file = new File(dirFilePath);
			if (file.isDirectory()) {
				String[] fileNames = file.list();
				List<String> list = new ArrayList<String>();
				for (int i = 0; i < fileNames.length; i++) {
					String filePath = dirFilePath + "/" + fileNames[i];
					if (vidateFileType(filePath, fileType)) {
						list.add(filePath);
					}
				}
				return list;
