...
public class PingYinUtil {
	...
	public static List<ExcelTemplateCol> getFirstSpellColList(List<ExcelTemplateCol> list) {
		List<ExcelTemplateCol> newList = new ArrayList<ExcelTemplateCol>();
		if (list == null || list.size() <= 0) {
			return list;
		}
		int size = 1;
		for (int i = 0; i < list.size(); i++) {
			ExcelTemplateCol col = list.get(i);
			for (int j = 0; j < newList.size(); j++) {
				if (col.getColumnName().equals(newList.get(j).getColumnName())) {
					col.setColumnName(col.getColumnName() + size);
					size++;
				}
			}
			newList.add(col);
