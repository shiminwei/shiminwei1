...
public class WebDataReportController {
	...
	public String toReport(HttpServletRequest request, Model model, SysDepartmentTemplate bean, String reportType,
		...
		// jsonConfigService.getExcelTemplateByName(bean.getTemplateName());
		if (StringUtil.isBlank(bean.getTemplateName())) {
			return "web/dataReport/report";
		}
		// try {
		// String tempalename = new
		// String(bean.getTemplateName().getBytes("ISO-8859-1"),"UTF-8");
	...
	public void checkExcelContent(HttpServletResponse response, HttpServletRequest request, Model model,
		...
		try {
			...
			// jsonConfigService.getExcelTemplateByName(bean.getTemplateName());
			ExcelTemplate template = excelTemplateMongoService.findByName(bean.getTemplateName());
			List<ProblemBean> list = ExcelUtils.checkExeclContent(file, template);
			request.setCharacterEncoding("utf-8");
			response.setContentType("text/html;charset=utf-8");
			response.setHeader("Cache-Control", "no-cache");
			PrintWriter out = response.getWriter();
	...
	public OpreateResult report(HttpServletResponse response, HttpServletRequest request, Model model,
		...
		try {
			...
			ExcelBean excelBean = ExcelUtils.readExcelByMultipartFile(file);
		//	ExcelBase.execlImpToDb(excelBean, template, user, webDataReportBean);
			// objList = ExcelUtil.replaceExeclContent(objList, template);// 替换
			String time = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss-SSS").format(new Date());
			System.out.println("插入SQL开始时间=====》" + time);
			// boolean flag = ExcelUtil.execlImpToDb(objList, template, user,
			// webDataReportBean);
			boolean flag = ExcelBase.execlImpToDb(excelBean, template, user, webDataReportBean);
			...
			if (flag) {
				...
				// sysReportLogService.addReportLog(user, webDataReportBean,
				// bean, template, objList.size());
				sysReportLogService.addReportLog(user, webDataReportBean, bean, template, excelBean.getContentSize());
			}
		} catch (Exception e1) {
			e1.printStackTrace();
		}
	...
	public void toDownload(HttpServletResponse response, HttpServletRequest request, Model model, String templateId) {
		try {
			if (StringUtil.isNotEmpty(templateId)) {
				ExcelTemplate template = excelTemplateMongoService.findById(templateId);
				ExcelUtils.toWriteExcelFileByTemplate(response, template);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String toReportList(HttpServletRequest request, Model model, SysDepartmentTemplate bean, String reportType,
			String year, String month) {
		if (StringUtil.isBlank(bean.getTemplateName())) {
			return "web/dataReport/reportList";
		}
		ExcelTemplate template = excelTemplateMongoService.findByName(bean.getTemplateName());
		Map<String, String> map = Constant.getMapByType(bean.getReportPeroid());
		model.addAttribute("year", Constant.getQueryYear());
	...
	public String reportContent(HttpServletRequest request,Model model,MultipartFile file){
		ExcelBean bean=ExcelUtils.readExcelByMultipartFile(file);
		model.addAttribute("bean", bean);
		return "web/dataReport/reportContent";
	}
