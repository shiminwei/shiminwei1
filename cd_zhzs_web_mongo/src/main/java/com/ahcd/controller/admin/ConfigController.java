...
public class ConfigController {
	 ...
	 */
	@RequestMapping("/list")
	public String getList(HttpServletRequest request, Model model, Page<ConfigBean> page, ConfigBean bean) {
		page = configService.getList(page, bean);
	...
	public String toSaveOrUpdate(HttpServletRequest request, Integer type, Model model, String id) {
		String returnUrl = "admin/config/edit";
		ConfigBean bean = configService.getConfigBeanById(id);
		if (type == 1) {
			List<DatasourceBean> datasourceList = jsonConfigService.getAllDatasource();
			model.addAttribute("datasourceList", datasourceList);
		} else if (type == 2) {
			model.addAttribute("fieldBeans", bean.getResults().getFieldBeans());
			List<ConfigBean> configList = configService.getAllList();
	...
	public String toEditCondition(Model model, String id) {
	...
	public OpreateResult addCondition(HttpServletRequest request, ConditionBean conditionBean, String id) {
		...
		if (configService.addCondition(conditionBean, id)) {
			opreateResult = new OpreateResult("200", "增加条件成功!", "conditionList", "closeCurrent", "");
		}
	...
	public OpreateResult toDelCondition(String id, String conditionId) {
		OpreateResult opreateResult = new OpreateResult("300", "删除条件失败", "conditionList", "", "");
		if (configService.delCondition(id, conditionId)) {
			opreateResult = new OpreateResult("200", "删除成功", "conditionList", "", "");
		}
		return opreateResult;
	}
	...
	public OpreateResult toUpOrDown(String id, String conditionId, String type) {
		...
		if (a == 1) {
			opreateResult = new OpreateResult("200", "操作成功!", "conditionList", "", "");
		} else if (a == 2) {
			opreateResult = new OpreateResult("300", "已经是第一个了!", "conditionList", "", "");
		} else if (a == 3) {
			opreateResult = new OpreateResult("300", "已经是最后一个了!", "conditionList", "", "");
		} else {
			opreateResult = new OpreateResult("300", "操作失败,缺少标识ID!", "conditionList", "", "");
		}
		return opreateResult;
	}
